...
hl.bind(mainMod .. " + TAB", hl.dsp.focus({ workspace = "previos" }))
...
hl.bind(mainMod .. " + Print", hl.dsp.exec_cmd(apps.shot))
...
hl.bind(mainMod .. " + XF86PowerOff", hl.dsp.exec_cmd("loginctl poweroff"))
hl.bind(mainMod .. " + XF86Sleep",    hl.dsp.exec_cmd("loginctl suspend"))
...
hl.bind(mainMod .. " + X", hl.dsp.exec_cmd("wpctl set-mute @DEFAULT_AUDIO_SOURCE@ toggle"))
...
hl.bind(mainMod .. " + G", function ()
    local name = hl.get_active_monitor().name
    local cmd = "wlr-randr "
    for i, mon in pairs(monitosList) do
        if mon ~= name then
            cmd = cmd .. " --output " .. mon .. " --toggle "
        end
    end
    hl.dispatch(hl.dsp.exec_cmd(cmd))
    hl.notification.create({ text = "Game mod on " .. name, timeout=2000 })
end)
...
for i = 1, 10 do
    local key = i % 10
    hl.bind(mainMod .. " + " .. key, function() hl.dispatch(hl.dsp.focus({ workspace = get_id(i) })) end)
    hl.bind(mainMod .. " + SHIFT + " .. key, function() hl.dispatch(hl.dsp.window.move({ workspace = get_id(i) })) end)
end
